...
<body>
    <div class="header">
        ...
        <p>Laporan Data Personel &mdash; {{ now()->format('d F Y, H:i') }}</p>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th style="width: 20%;">Nama Personel</th>
        ...
        <tbody>
            @foreach($personels as $index => $personel)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td><strong>{{ $personel->code }}</strong></td>
                    <td>{{ $personel->name }}</td>
                    <td>{{ Str::limit($personel->competencies->map(function($c) { return $c->name . ' ' . $c->level_label; })->implode(', '), 60) }}</td>
                    <td>{{ $personel->competencies->count() }}</td>
                    <td>
                        <span class="badge {{ $personel->is_active ? 'badge-active' : 'badge-inactive' }}">
