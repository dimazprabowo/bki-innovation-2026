@props(['evidences', 'deletingEvidenceId'])
                    ...
                    @if($isExisting)
                        ...
                        <div class="flex items-center gap-3 p-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <button type="button"
                                    wire:click="confirmDeleteEvidence('{{ $evidence['id'] }}')"
                                    wire:loading.attr="disabled"
                                    wire:target="confirmDeleteEvidence('{{ $evidence['id'] }}')"
                                    class="text-red-400 hover:text-red-600 p-1 disabled:opacity-50" title="Hapus evidence">
                                    <svg wire:loading.class="hidden" wire:target="confirmDeleteEvidence('{{ $evidence['id'] }}')" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                    <svg wire:loading wire:target="confirmDeleteEvidence('{{ $evidence['id'] }}')" class="animate-spin w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                    </svg>
                                </button>
